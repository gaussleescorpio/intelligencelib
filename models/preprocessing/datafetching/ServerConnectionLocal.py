...
Module description:
Users can use this module to connect to bisam-client01 server by initializing the data retrieval process.
Base virtual class, derived data retrieval class and implementation examples are all given below.

Classes:
VirtualMysqlFunctions: a virtual class from which users can freely derive new classes
IbeFetcher: a generic class for retrieving data based on VirtualMysqlFunctions

Functions and methods:
1. for retrieving all of the data, you should use function read_data_by_selection.
2. for retrieving data with known indexes, you should use function read_data_by_selection_index.
   The index can be sql datatime, such as a string "2013-01-01" or a int index
   which is used to quote different rows in the sql database.
3. for getting heads and tails of the data from a particular table.
   You can use Read_data_head or Read_data_tails. Please note that these two functions
...
class VirtualMysqlFunctions(object):
    """
    virtual class for all users. Only provide a format of intended classes.
    Any pure virtual function should have an implementation in
    ...
    def use_database(self):
        ...
        pass
    @abc.abstractmethod
    def retrieve_data_by_selection(self):
        """
...
class IbeFetcher(VirtualMysqlFunctions):
    """
    This class mainly works for one single database,
    if you wanna information across different databases, please use the "retrieving data with commands"
    ...
    def __init__(self,hostname="bisam-client01", username="gauss", password="gauss"):
        ...
        :param hostname: the remote ip of host
        :param username:
        :param password:
        :exception: operational error handling.
        """
        ...
        con_success = True
        try:
            self.db = mdb.connect(host=hostname, user=username,passwd=password, db="ibemining")
    ...
    def database_exists(self, db_name = "d"):
        """
    ...
    def retrieve_data_by_selection(self, con="*", tb_name=None):
        """
        This function is used to access the database with small size.
        For big size data retrieval, please use Read_data_by_selection_index
        ...
        if not con:
            ...
            if self.database_name is not None and self.table_exists(tb_name):
    ...
    def retrieve_data_by_selection_index(self, con="*", tb_name=None, start_index=None, end_index=None, index_name="Default"):
        """
    ...
    def retrieve_data_head(self,con="*", num=1, tb_name=None):
        """
        ...
        :return: head n number of data
        """
        commands = """ select %s from %s  """ %(con,tb_name)
        dict_rec = self.retrieve_data_with_commands(commands)
        return dict_rec.head(num)

    def retrieve_data_tails(self, con="*", num = 1, tb_name=None):
        """
        Do not assign very big num with huge tables to this function, it will be slow
        :param con:  a string composed of all the retrieving attrs or cols, ex: "attr1, attr2"
        :param num: num of head elements you want to get
        :param tb_name:
        :return: tail n number of data
        """
        commands = """ select %s from %s  """ %(con,tb_name)
        dict_rec = self.retrieve_data_with_commands(commands)
        return dict_rec.tail(num)

    def retrieve_data_with_commands(self, command=""):
        """
        Generic sql command exec function
        :param command: any sql command
        :return: data retrieved
        :exception: empty data error handling
        """
...
if __name__ == "__main__":
    ...
    print server.table_exists("d_air_bookings")
    a = server.retrieve_data_by_selection("NAME,CITY_ID",tb_name="r_airports")

    print server.retrieve_data_by_selection_index(con="Order_ID",tb_name="d_air_bookings",start_index ="2014-02-01",end_index="2014-03-01",index_name="Order_Created" )
    print server.retrieve_data_with_commands(command="""select NAME from r_airports""")
    # commands = """select Order_Created, Departure_Time_First_Segment from d_air_bookings
    # where Order_Created between '2016-03-01' and '2016-03-03' """
    # print server.Get_data_with_commands(command=commands)
    print server.retrieve_data_head(tb_name="r_airports",con="NAME,CITY_ID",num=100)
    print server.retrieve_data_tails(tb_name="r_airports",con="NAME,CITY_ID",num=100)